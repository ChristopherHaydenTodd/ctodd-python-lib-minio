...
from minio_helpers.minio_exceptions import BucketAlreadyExists
...
def main():
    ...
    if opts.bucket_name in bucket_names:
        logging.info(f"{opts.bucket_name} already exists, expecting an exception")

    try:
        minio_bucket_helpers.create_bucket(minio_client, opts.bucket_name)
    except BucketAlreadyExists as ba_err:
        logging.error(f"Got expected Error: {ba_err}")
    except Exception as err:
        logging.error(f"Unexpected Error Creating Bucket: {err}")
        raise err
...
def get_options():
    ...
    parser = ArgumentParser(description="Create Bucket in Minio")
